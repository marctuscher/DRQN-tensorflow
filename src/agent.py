...
class Agent():
    ...
    def __init__(self, train_steps=10000000 ,batch_size=32, history_len=4, mem_size=500000, frame_skip=1, epsilon_start=1, epsilon_end=0.1,
                 epsilon_decay_episodes=100000, screen_height=84, screen_width=84, train_freq=4, update_freq=10000,
        ...
        self.env_wrapper = GymWrapper('Breakout-v0', self.screen_height, self.screen_width, frame_skip=frame_skip)
    ...
    def train(self, steps):
        ...
        for self.i in tqdm(range(self.i, steps)):
            ...
            if self.i % self.train_freq == 0 and self.i > self.train_start:
                total_q += self.net.train_on_batch_all_tf(*self.replay_memory.sample_batch(), self.i)
                train_count += 1.0
